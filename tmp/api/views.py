...
# allows us to view and create user model for each user. (user holds all user data, displayed on user page)
...
class SortingPageView(APIView):
    ...
    def post(self, request, format=None):
        ...
        # user create user serailizer to check if request data valid
        serializer = SortingPageSerializer(data=request.data)
        if serializer.is_valid():
            sorting_criteria = serializer.data.get('sorting_criteria')
            user = self.request.session.session_key

            # if user already has a user model and tries to make a new one (wants to resort),
            # there model will have the same code but with the updated settings
            ...
            # if not updating the user model create a new one! 
            else:
            ...
            # return user model by serializing it
